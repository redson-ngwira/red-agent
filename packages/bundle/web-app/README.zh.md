...
运行 `dsh --profile web`，界面会在你的默认浏览器中打开，即可与 agent（智能体）交互式聊天。你会获得会话视图、模型与设置管理以及会话历史，背后与其他表层相同的模型访问、工具与安全默认值。该命令会打印带 token 的启动 URL；浏览器用该 token 换取签名会话 cookie，再重定向到干净的根 URL。你可以从命令行更改端口、关闭浏览器交接并允许额外主机；`--host 0.0.0.0` 会绑定所有网卡以支持托管部署（Render 等），设置 `PORT` 时默认为 `0.0.0.0:$PORT`。需要浏览器中的交互式工作时选择它；`dsh-headless` 是一次性的命令行兄弟表层。
...
- **支持绑定所有网络接口**——`--host 0.0.0.0` 会绑定所有网卡以支持托管部署；设置 `PORT` 时默认为 `0.0.0.0`（Render），否则为 `127.0.0.1`（本地）。公网绑定需配合身份验证与 `trustedHosts` 使用。
