...
describe('FishLogo', () => {
  it('renders the RED AGENT R badge (kept as FishLogo for API compat)', () => {
    const { container } = render(<primitives.FishLogo />)
    expect(container.textContent).toContain('R')
    expect(container.innerHTML).toContain('#E10600')
  })
})

describe('BrandWordmark', () => {
  it('can render the RED AGENT wordmark with or without its leading mark', () => {
    const view = render(<primitives.BrandWordmark />)
    expect(view.container.textContent).toContain('RED AGENT')
    expect(view.container.innerHTML).toContain('#E10600')
